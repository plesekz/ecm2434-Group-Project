...
def take_turn(player, target):
    for i in range(0, player.num):
        ...
        return (
            f"Attack {i} rolled {roll} against {(player.acc - target.ev)}, leaving opponent on {target.hp}")
...
def callBattle(request):
    ...
    if log[0]:
        # with stats hp {p.hp}, tg {p.tg}, ev {p.ev}, dmg {p.dmg}, acc {p.acc},
        # num  {p.num}, log <br> {log[1]}'))
        messages.success(request, (f'Victory'))
        return redirect("/battleSelect")
    # with stats hp {p.hp}, tg {p.tg}, ev {p.ev}, dmg {p.dmg}, acc {p.acc},
    # num  {p.num}, log <br> {log[1]}'))
    messages.success(request, (f'Loss'))
    return redirect("/battleSelect")
